...
class ReportService {
  ...
  static Future<String> generatePdfReport(FinanceController finance) async {
    ...
    final formattedPrintDate =
        "${printDate.day}/${printDate.month}/${printDate.year} ${printDate.hour}:${printDate.minute.toString().padLeft(2, '0')}";
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
                ...
                children: [
                  ...
                  pw.Row(
                    ...
                    children: [
                      ...
                      pw.Text(
                        'Profesi: ${profile.profession.isNotEmpty ? profile.profession : "-"}',
                        style: const pw.TextStyle(fontSize: 10),
                      ),
                    ],
                  ),
                    ...
                    children: [
                      ...
                      pw.Text(
                        ...
                        style: pw.TextStyle(
                          fontSize: 10,
                          fontWeight: pw.FontWeight.bold,
                          color: finance.overallRemainingBalance >= 0
                              ? PdfColors.green
                              : PdfColors.red,
                        ),
                      ),
            ...
            pw.Text(
              'RINGKASAN ALOKASI ANGGARAN (METODE 6 POS)',
            ...
            pw.Table(
              ...
              children: [
                ...
                // Rows
                _buildPdfBudgetRow(
                  'Survival (Bertahan Hidup - 40%)',
                  finance.limitSurvival,
                  finance.spendingSurvival,
                  finance.remainingSurvival,
                  finance.getCategoryStatus('Survival')['status'],
                ),
                _buildPdfBudgetRow(
                  'Transportasi (10%)',
                  finance.limitTransport,
                  finance.spendingTransport,
                  finance.remainingTransport,
                  finance.getCategoryStatus('Transportasi')['status'],
                ),
                _buildPdfBudgetRow(
                  'Gaya Hidup / Style (10%)',
                  finance.limitStyle,
                  finance.spendingStyle,
                  finance.remainingStyle,
                  finance.getCategoryStatus('Style')['status'],
                ),
                _buildPdfBudgetRow(
                  'Hiburan (10%)',
                  finance.limitEntertainment,
                  finance.spendingEntertainment,
                  finance.remainingEntertainment,
                  finance.getCategoryStatus('Hiburan')['status'],
                ),
                _buildPdfBudgetRow(
                  'Dana Darurat (10%)',
                  finance.limitEmergency,
                  finance.spendingEmergency,
                  finance.remainingEmergency,
                  finance.getCategoryStatus('Dana Darurat')['status'],
                ),
                _buildPdfBudgetRow(
                  'Goals & Investasi (20%)',
                  finance.limitGoals,
                  finance.totalSavingsInGoals,
                  finance.remainingGoals,
                  finance.getCategoryStatus('Goals')['status'],
                ),
              ],
            ),
            pw.SizedBox(height: 24),
            ...
            // GOALS SECTION
            pw.Text(
              'IMPIAN KEUANGAN & TARGET TABUNGAN',
              style: pw.TextStyle(fontSize: 12, fontWeight: pw.FontWeight.bold),
            ),
            pw.SizedBox(height: 8),
            if (goals.isEmpty)
              pw.Text(
                'Belum ada impian keuangan yang terdaftar.',
              ...
              pw.Table(
                ...
                children: [
                  ...
                  ...goals.map((g) {
                    ...
                    final remStr = remaining > 0
                        ? _formatIDR(remaining)
                        : "Lunas 🎉";
                    return pw.TableRow(
                      children: [
                        pw.Padding(
    ...
    final path =
        "${directory.path}/finsim_laporan_${printDate.millisecondsSinceEpoch}.pdf";
    final file = File(path);
    await file.writeAsBytes(await pdf.save());
    return path;
  }

  static pw.TableRow _buildPdfBudgetRow(
    String pos,
    double limit,
    double spending,
    double remaining,
    ...
    final statusColor = status == 'Sehat'
        ? PdfColors.green800
        : PdfColors.red800;
    return pw.TableRow(
      children: [
        pw.Padding(
          padding: const pw.EdgeInsets.all(6),
          child: pw.Text(pos, style: const pw.TextStyle(fontSize: 8.5)),
        ),
        pw.Padding(
          padding: const pw.EdgeInsets.all(6),
          child: pw.Text(
  ...
  static Future<String> generateExcelReport(FinanceController finance) async {
    ...
    overviewSheet
        ...
        .value = TextCellValue(
      'Sisa Saldo Bersih: ${_formatIDR(finance.overallRemainingBalance)}',
    );
    overviewSheet
        .cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: 5))
        .value = TextCellValue(
      'Tanggal Cetak: ${DateTime.now()}',
    );
    ...
    for (int i = 0; i < budgetRows.length; i++) {
      ...
      final rIndex = 8 + i;
      overviewSheet
          .cell(CellIndex.indexByColumnRow(columnIndex: 0, rowIndex: rIndex))
          .value = TextCellValue(
        r[0] as String,
      );
      overviewSheet
          .cell(CellIndex.indexByColumnRow(columnIndex: 1, rowIndex: rIndex))
          .value = DoubleCellValue(
        r[1] as double,
      );
      overviewSheet
          .cell(CellIndex.indexByColumnRow(columnIndex: 2, rowIndex: rIndex))
          .value = DoubleCellValue(
        r[2] as double,
      );
      overviewSheet
          .cell(CellIndex.indexByColumnRow(columnIndex: 3, rowIndex: rIndex))
          .value = DoubleCellValue(
        r[3] as double,
      );
      overviewSheet
          .cell(CellIndex.indexByColumnRow(columnIndex: 4, rowIndex: rIndex))
          .value = TextCellValue(
        r[4] as String,
      );
    }
    ...
    for (int i = 0; i < goals.length; i++) {
      ...
      goalsSheet
          .cell(CellIndex.indexByColumnRow(columnIndex: 5, rowIndex: rIndex))
          .value = TextCellValue(
    ...
    final directory = await getApplicationDocumentsDirectory();
    final path =
        "${directory.path}/finsim_laporan_${DateTime.now().millisecondsSinceEpoch}.xlsx";
    final file = File(path);
